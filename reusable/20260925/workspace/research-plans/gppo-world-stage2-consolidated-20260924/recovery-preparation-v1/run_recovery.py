...
from recovery_contract import cumulative_snapshot, reject_duplicate_or_wrong_order
OLD=ROOT.parent
...
class Monitor:
    ...
    def check(self,disk=False):
        ...
        if disk:
            size=self.disk();stage_cap=38*1024**3
...
def stages_environment(stage,auth,monitor,reservations,native):
    ...
    try:
        ...
        if executed!=remaining_keys or len(episodes)!=5566:raise RuntimeError('Incomplete recovery suffix')
...
def publish_progress(monitor, completed, key):
    ...
    try:
        with path.open('a',encoding='utf-8') as f:
            f.write(canonical({'recovery_completed':completed,'total_completed':4034+completed,'planned':9600,'last_key':key})+'\n')
            f.flush()
    except OSError:
...
def cumulative_resources(monitor,request):
    ...
    value['guard_holdback_wall_seconds']=request['shutdown_holdback']['wall_seconds']+request['recovery_shutdown_holdback']['wall_seconds']
    value['guard_holdback_cpu_seconds']=request['shutdown_holdback']['cpu_seconds']+request['recovery_shutdown_holdback']['cpu_seconds']
...
def main(argv=None):
    ...
    write(out/'ledger-lineage.json',{'origin_sqlite':str(OLD_RUN/'budget.sqlite3'),
        'origin_sha256':sha(OLD_RUN/'budget.sqlite3'),'historical_stages':old_stages,
        'additional_model_loads_explicitly_authorized':1,'cannot_refund_history':True})
    limits=limits_for(request)
    budget=budget_class(native)(out/'budget.sqlite3',limits=limits,run_limits=limits,
        attempt_id='consolidated-v1-recovery-1-once',run_id='consolidated-v1-recovery-1')
    ...
    try:
        ...
        retained=read(ROOT/'retained-episodes.json');combined=retained+episodes
        if len(combined)!=9600:raise RuntimeError('Original matrix incomplete')
        original_cost=read(ROOT/'retained-cost-inputs.json')
        resources=cumulative_resources(monitor,request)
        merged=merge_cost(original_cost,cost,resources)
        ...
        snapshot=budget.snapshot();cumulative=cumulative_snapshot(old_stages,snapshot)
        ...
        write(out/'status.json',{'status':'completed','decision':result['decision'],
            ...
            'stage3_to5_authorized':False,'automatic_retry':False})
        ...
        write(out/'status.json',{'status':'technical_stop','error':f'{type(exc).__name__}: {exc}',
            'segment_budget':snapshot,'cumulative_stages':cumulative_snapshot(old_stages,snapshot),
            'resources':cumulative_resources(monitor,request),'automatic_retry':False})
        raise
