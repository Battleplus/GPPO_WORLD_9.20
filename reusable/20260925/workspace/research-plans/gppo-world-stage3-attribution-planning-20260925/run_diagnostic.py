...
def validate_matrix(rows,baseline,tapes):
    expected=[(f'eval-{p:04d}',r,'N') for p in range(1600) for r in range(3)]
    if [(r['parent'],r['repeat'],r['arm']) for r in rows]!=expected:raise RuntimeError('Frozen N matrix order mismatch')
    old={(r['parent'],r['repeat'],r['arm']):r for r in baseline}
    if len(old)!=9600 or len(baseline)!=9600 or set(old)!={(p,r,a) for p,r,_ in expected for a in ('M','R')}:
        raise RuntimeError('Historical matrix mismatch')
    ...
    for index,row in enumerate(rows):
        p,r,_=expected[index]
        if row['tape_index']!=index//3 or row['tape_hash']!=tapes['hashes'][index//3]:raise RuntimeError('Wrong tape mapping')
        if row['exogenous_key']!=f'frozen-native-public-history-allocation-v1|{p}|repeat-{r}':raise RuntimeError('Wrong exogenous key')
        if not row['initial_public_hash']==old[(p,r,'M')]['initial_public_hash']==old[(p,r,'R')]['initial_public_hash']:
            raise RuntimeError('Historical initial pairing mismatch')
...
def main(argv=None):
    ...
    try:
        ...
        old=read(request['inputs']['baseline_episodes']);prior=read(request['inputs']['baseline_analysis'])
        cost['historical_M_mean_cpu_ms']=prior['controller_cost']['M']['mean_cpu_ms']
        resources=monitor.snapshot()
        result=analyze(old,episodes,cost,a,resources,check=lambda:monitor.check())
