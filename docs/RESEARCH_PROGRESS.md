## 2026-09-23 — model-rule-stage1 execution package

Prepared and statically validated the frozen-model versus public-only rule comparison package. Dynamic execution remains pending explicit user approval. The package fixes and validates normalized public rank carriers, fail-closed legal-mask contradictions, selector provenance, zero model calls on the rule arm, and parent-level utility analysis. The archive excludes checkpoint weights, prefix pickle, SQLite, and resolved training config.

# 当前最新状态：H-006 固定事件特征干预实验完成

48/48分支、24/24配对；normal/off主偏好效用均0.27682090172392726，父场景宏平均差0.0，95%父级bootstrap CI [0.0,0.0]。所有8个parent的3repeat差值均为0；按冻结标准为inconclusive，不能宣称等效或事件输入一般无用。

输入干预确实生效：首对TV=0.002435835；额外描述性核对279个同输入记录中，230个概率分布改变、7个完整合法排名改变，top-1和guard最终选择均0次改变。任务、能耗、动作轨迹结果一致。两臂均使用冻结GPPO/world+guard，不能称为整个世界模型净收益或独立泛化。

新增558环境步，policy/world各558、actor读出560，全部更新0。用户批准后原SQLite上限404→1067，历史299保留，最终857 reserved/verified，pending/unknown0，integrity ok；未退款、补样或重跑。总wall133.94秒，日志计时存在不同边界，未声称计算节省。

研究决策：封存本固定开发矩阵干预实验；未观察到最终动作或效用增量，不补样追正、不据此删除训练组件。唯一后续建议为静态定义同合同公开信息的无模型规则+guard对照，任何新动态工作仍需协议与授权。本轮未启动训练、偏好、事件触发或泛化。整体目标仍未完成。

[最终报告](archive/H-20260923-GPPO-WORLD-MAINLINE-006-event-feature-results/report.md) · [父场景结果](archive/H-20260923-GPPO-WORLD-MAINLINE-006-event-feature-results/parent-results.json) · [原分析](archive/H-20260923-GPPO-WORLD-MAINLINE-006-event-feature-results/run/analysis.json) · [集中审查](archive/H-20260923-GPPO-WORLD-MAINLINE-006-event-feature-results/review.json) · [原始制品索引](archive/H-20260923-GPPO-WORLD-MAINLINE-006-event-feature-results/artifact-manifest.json)。原始大ledger、模型、SQLite保留本地；旧失败与勘误不覆盖。

以下保留历史进度，旧“当前状态”不是最新结论。


---

# 当前最新状态：H-006 执行包 v2 修正，动态授权待定

上一提交 dd2a542 的远端回读确实成功，但执行包完成判断过早：首对门控和额外读出未接入实际入口，旧30项测试未覆盖。该申请已撤回；本修正包及新增入口测试取代旧包，旧提交和报告保留，不作为算法结果。详见 [勘误](archive/H-20260923-GPPO-WORLD-MAINLINE-006-execution-package/erratum-v2.md)。

整体研究目标 H-20260923-GPPO-WORLD-MAINLINE-006 保持进行中。阶段一集中预检已完成；本轮补齐固定48分支的运行器、只读分析器、同账本授权迁移工具、纯/stub测试及源身份。未运行环境、恢复快照、模型推理、训练或新增正式attempt。

唯一比较：normal 与 event_features_off，均使用同一冻结 GPPO/world 和公开执行状态 guard。仅关闭actor输入末5维事件特征；零不是已证明的分布内中性值，因此结论只限指定干预敏感性，不能解释为整个世界模型的净收益。H005基线含冻结模型，不能误写成已验证的无模型控制器。

固定8个已观察开发parent ×3repeat ×2arm，最多768新环境步、768policy encode、768world候选batch及770actor readout，所有更新0。原SQLite仍299/404，105剩余额度未使用；待用户明确批准后，才允许同库上限404→1067（增加663），新run上限768。没有新建替代预算。

[执行报告](archive/H-20260923-GPPO-WORLD-MAINLINE-006-execution-package/report.md) · [完整计划](archive/H-20260923-GPPO-WORLD-MAINLINE-006-execution-package/EXPERIMENT_PLAN.md) · [运行命令](archive/H-20260923-GPPO-WORLD-MAINLINE-006-execution-package/COMMANDS.md) · [哈希](archive/H-20260923-GPPO-WORLD-MAINLINE-006-execution-package/hashes.json)。本轮没有效果结论，不进入偏好、事件触发或泛化阶段。

以下历史进度原样保留；旧“当前任务”标题不代表最新状态。

---

# 当前最新状态：H-006 阶段一执行包已准备，动态授权待定
