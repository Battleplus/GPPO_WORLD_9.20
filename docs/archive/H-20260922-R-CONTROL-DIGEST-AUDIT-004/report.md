...
See `root-cause-report.json`, `schema-comparison.json`, `bridge-evidence.json`, `compatibility-strict.json`, `compatibility-bridge-qualified.json`, `branch-statuses.json`, `source-input-identities.json`, `validation.json`, and `independent-verification.json`. The actual source patch relative to the frozen phase-1 runner is `minimal-diff.patch`; it contains the runner bridge hook plus the new schema helper and its pure tests.
