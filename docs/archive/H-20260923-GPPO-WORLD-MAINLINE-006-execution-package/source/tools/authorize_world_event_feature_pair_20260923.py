...
PACKAGE = ROOT / "runs/finite-communication-ack-lease-fix-20260920/world-event-feature-execution-package-20260923"
DATABASE = PACKAGE.parent / "ack-known-task-guard-baseline-v1/budget.sqlite3"
OLD_SHA = "9ff13f0e18ab8ce56f418f00eac786f08a47e6107a2b8d31f7184b2502c9d420"
...
def validate_approval(approval, package_hash):
    ...
    require(approval.get("handoff_id")=="H-20260923-GPPO-WORLD-MAINLINE-006","wrong objective")
    ...
    require(approval.get("accepted_event_sensitivity_only") is True,"interpretation not approved")
    require(approval.get("old_limit")==404 and approval.get("new_limit")==1067 and approval.get("new_step_cap")==768,"approved budget scope mismatch")
    require(approval.get("model_call_caps")=={"policy_encode":768,"world_candidate_batch":768,"actor_readout":770},"model call scope mismatch")
...
def extend_same_database(path, expected_sha, backup, receipt, approval_hash):
    ...
    try:
        ...
        require(states.get("environment_steps")== (404,299,299,0),"old stage state mismatch")
        ...
        cursor=con.execute("UPDATE stages SET limit_amount=1067 WHERE stage='environment_steps' AND limit_amount=404 AND reserved=299 AND verified=299 AND unknown=0")
        require(cursor.rowcount==1,"limit update did not match one row")
        after=logical_rows(con)
        expected=json.loads(json.dumps(before))
        for row in expected["stages"]:
            if row[0]=="environment_steps":row[1]=1067
    ...
    result={"status":"applied","database":str(path),"before_sha256":expected_sha,"after_sha256":sha(path),
            ...
            "only_change":"environment_steps limit404->1067","old_history_preserved":True,
            "historical_reserved_verified":299,"new_attempt":0,"env_step":0,"model_forward":0,"all_updates":0}
...
def main():
    ...
    runner_path=ROOT/"tools/run_world_event_feature_pair_20260923.py"
