...
class RuntimeAdapters:
    ...
    # A model probe performs one policy encode, world candidate batch, and
    # actor readout.  A public-rule probe performs none of these calls.  Keep
    # this declaration on the adapter so counters describe the actual probe,
    # rather than assuming every selector is a learned model.
    model_calls_per_probe: Mapping[str, int] = field(
    ...
    def declared_model_calls(self) -> dict[str, int]:
        names = ("policy_encode", "world_candidate_batch", "actor_readout")
        declaration = self.model_calls_per_probe
        if isinstance(declaration, Mapping):
            values = {name: int(declaration.get(name, 0)) for name in names}
        else:
            try:
                values = {name: int(value) for name, value in zip(names, declaration)}
            except (TypeError, ValueError):
                raise RuntimeTechnicalStop(f"invalid model call declaration: {declaration!r}")
            if len(values) != len(names):
                raise RuntimeTechnicalStop(f"invalid model call declaration: {declaration!r}")
        if any(value < 0 for value in values.values()):
            raise RuntimeTechnicalStop(f"negative model call declaration: {values}")
        return values
...
def _execute_branch_impl(
    ...
    while not done:
        ...
        try:
            ...
            declared_calls = adapters.declared_model_calls()
            counters.model_forward += sum(declared_calls.values())
            counters.actor_forward += declared_calls["actor_readout"]
            counters.world_forward += declared_calls["world_candidate_batch"]
        ...
        if not isinstance(selected_action_record, Mapping) or "hidden" not in selected_action_record:
            failure = {
                ...
                "env_step_calls": branch_count("env_step_calls"),
                "model_call_status": {
                    name: {"attempted": count, "completed": count}
                    for name, count in declared_calls.items()
                },
            }
            append_jsonl(out_dir / "failure-ledger.jsonl", failure)
