...
spec=importlib.util.spec_from_file_location('authorize_pair',Path(__file__).resolve().parents[1]/'tools/authorize_model_rule_pair_v1.py')
m=importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)

def approval():
    return dict(status='approved',experiment_id='model-rule-stage1-v1',package_hashes_sha256='package',approved_by='user',user_message_reference='test-only',approved_at='2026-09-23',accepted_model_rule_development_scope=True,old_limit=1067,new_limit=1625,new_step_cap=768,model_call_caps=dict(policy_encode=384,world_candidate_batch=384,actor_readout=384),rule_selector_cap=384,all_updates=0)
...
def database(tmp_path,pending=False):
    ...
    with sqlite3.connect(path) as c:
        ...
        c.executemany('INSERT INTO stages VALUES (?,?,?,?,?)',[('environment_steps',1067,857,857,0),('optimizer_calls',0,0,0,0),('world_updates',0,0,0,0),('offline_updates',0,0,0,0)])
...
def test_approval_rejects_missing_or_expanded_scope():
    ...
    for key,value in [('status','pending'),('approved_at',''),('new_limit',1626),('all_updates',1),('package_hashes_sha256','other'),('accepted_model_rule_development_scope',False)]:
...
def test_extension_changes_only_stage_limit_and_preserves_backup(tmp_path):
    ...
    before['stages']=[([r[0],1625,*r[2:]] if r[0]=='environment_steps' else r) for r in before['stages']]
    assert after==before and receipt['new_attempt']==0
    with sqlite3.connect(tmp_path/'backup.sqlite3') as c:
        assert c.execute("SELECT limit_amount FROM stages WHERE stage='environment_steps'").fetchone()==(1067,)
...
def test_invalid_state_never_mutates_database(tmp_path,fault):
    ...
    if fault=='state':
        with sqlite3.connect(path) as c:c.execute("UPDATE stages SET reserved=858 WHERE stage='environment_steps'")
    before=m.sha(path)
