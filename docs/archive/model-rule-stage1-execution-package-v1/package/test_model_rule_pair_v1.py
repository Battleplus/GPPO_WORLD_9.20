...
import tools.analyze_model_rule_pair_v1 as ANALYZER
...
def test_stub_runner_output_is_accepted_by_analyzer_runtime_contract(monkeypatch, tmp_path):
    gate = _stub_gate(tmp_path)
    monkeypatch.setattr(RUNNER, "validate_authorization", lambda *args, **kwargs: copy.deepcopy(gate))
    budget = _Budget()
    monkeypatch.setattr(RUNNER, "_load_module", lambda path, name: RUNTIME if Path(path) == RUNNER.RUNTIME else __import__("gppo_world.public_dispatch_rule_v1", fromlist=["*" ]))
    output = tmp_path / "runner-analyzer-contract"
    RUNNER.execute_authorized(
        tmp_path / "auth.json", manifest_path=RUNNER.MANIFEST, output_dir=output,
        adapters=_stub_adapters(), budget_factory=lambda runtime, auth: budget,
    )
    manifest = RUNNER._manifest_rows(RUNNER.MANIFEST)
    selector = RUNNER.read_jsonl(output / "selector-ledger.jsonl")
    decisions = RUNNER.read_jsonl(output / "decision-ledger.jsonl")
    steps = RUNNER.read_jsonl(output / "step-vector-rewards.jsonl")
    ...
    first_gate = json.loads((output / "first-pair-gate.json").read_text(encoding="utf-8"))
    evidence = ANALYZER.validate_runtime_evidence(selector, decisions, steps, manifest, status, costs, first_gate)
    assert evidence["rule_selector_calls"] == 24
    assert evidence["model_call_counts"] == {name: 24 for name in RUNNER.ModelCallCounters.NAMES}
