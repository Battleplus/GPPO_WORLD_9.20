...
class M10Environment:
    ...
    def _reset_state(self) -> None:
        ...
        self._contract_last_heartbeat = -math.inf
    ...
    def _contract_should_report(self, kind: str, entity: str, field: str,
        ...
        heartbeat = (now - self._contract_last_heartbeat >=
                     self.config.contract_v1.heartbeat_interval)
    ...
    def _deliver_observations(self) -> None:
        now = self.clock.time
        contract = self.config.contract_v1
        heartbeat_due = bool(contract and contract.reporting_mode == "event" and
                             now - self._contract_last_heartbeat >= contract.heartbeat_interval)
        for uid, resource in self.clock.resources.items():
            ...
            for field, value in values.items():
                if self._contract_should_report("uav", uid, field, value, now, force=heartbeat_due):
        ...
        for task_id, task in self.clock.tasks.items():
            ...
            for field, value in values.items():
                force = heartbeat_due or emergency_ready
        ...
        if heartbeat_due:
            self._contract_last_heartbeat = now
        self._flush_contract_pending_sends()
