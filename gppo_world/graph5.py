...
def graph5_from_m10_observation(observation: Mapping[str, Any], *, counts: tuple[int, int, int, int, int] = (4, 3, 4, 6, 4)) -> Graph5Snapshot:
    ...
    return Graph5Snapshot(nodes=nodes, candidate_features=relations.reshape(GRAPH5_CANDIDATE_COUNT, 4), action_mask=observation["mask"], graph_version=int(observation["version"]))


__all__ = ["GRAPH5_TYPES", "GRAPH5_NODE_DIM", "GRAPH5_CANDIDATE_COUNT", "GRAPH5_ACTION_COUNT", "Graph5Snapshot", "graph5_from_dict", "graph5_from_m10_observation"]
